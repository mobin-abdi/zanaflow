...
int main()
{
    ...
    if (!layer)
        fail("zf_dense_create returned NULL");

    int in_shape[] = {1, 2};
    Tensor *input = zf_tensor_create(in_shape, 2);
    if (!input)
        fail("zf_tensor_create(input) returned NULL");
    ...
    layer->bias->value->data[0] = 0.1f;
    layer->bias->value->data[1] = 0.2f;
    layer->bias->value->data[2] = 0.3f;

    Tensor *output = zf_dense_forward(layer, input);
    if (!output)
        fail("zf_dense_forward returned NULL");

    if (output->ndim != 2)
        fail("output ndim should be 2");
